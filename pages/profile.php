...
if (isset($_POST['cancel_order'])) {
    ...
    if (mysqli_query($conn, $query)) {
        header("Location: profile.php?success=Đơn hàng đã được hủy thành công!");
        exit();
    } else {
...
<head>
    ...
    <title>Hồ Sơ Người Dùng - BABY Store</title>
...
<body>
    <section class="py-6 mt-6">
        <div class="container mx-auto px-4">
            <h2 class="text-4xl font-bold text-center bg-gradient-to-r from-pink-400 to-blue-400 bg-clip-text text-transparent mb-8">Hồ Sơ Người Dùng</h2>
            ...
            <div class="grid md:grid-cols-2 gap-6">
                ...
                <div class="glass-container rounded-lg shadow-lg p-6">
                    <h3 class="text-2xl font-semibold bg-gradient-to-r from-pink-400 to-blue-400 bg-clip-text text-transparent mb-4">Thông Tin Cá Nhân</h3>
                    <?php if ($user) { ?>
                        ...
                        <div class="mt-6">
                            <button class="edit-button hover-scale" onclick="toggleEditForm()">Chỉnh Sửa Hồ Sơ</button>
                        </div>

                        <!-- Form chỉnh sửa hồ sơ (ẩn mặc định) -->
                        <div id="editForm" class="mt-6 hidden">
                            <h4 class="text-lg font-semibold mb-4">Chỉnh Sửa Thông Tin</h4>
                            <form method="POST" action="">
                                <div class="grid grid-cols-1 gap-4">
                                    <div>
                                        <label class="block text-gray-700 font-semibold mb-2">Họ Tên</label>
                                        <input type="text" name="name" value="<?php echo htmlspecialchars($user['username']); ?>" class="w-full p-3 bg-gray-100 rounded-lg border-none outline-none text-sm" required>
                                    </div>
                                    <div>
                                        <label class="block text-gray-700 font-semibold mb-2">Email</label>
                                        <input type="email" name="email" value="<?php echo htmlspecialchars($user['email']); ?>" class="w-full p-3 bg-gray-100 rounded-lg border-none outline-none text-sm" required>
                                    </div>
                                    <div>
                                        <label class="block text-gray-700 font-semibold mb-2">Số Điện Thoại</label>
                                        <input type="text" name="phone" value="<?php echo htmlspecialchars($user['phone']); ?>" class="w-full p-3 bg-gray-100 rounded-lg border-none outline-none text-sm" required>
                                    </div>
                                    <div>
                                        <label class="block text-gray-700 font-semibold mb-2">Địa Chỉ</label>
                                        <input type="text" name="address" value="<?php echo htmlspecialchars($user['address'] ?? ''); ?>" class="w-full p-3 bg-gray-100 rounded-lg border-none outline-none text-sm">
                                    </div>
                                </div>
                                <div class="mt-6">
                                    <button type="submit" class="edit-button hover-scale">Lưu Thay Đổi</button>
                                    <button type="button" class="ml-4 bg-gray-400 text-white px-4 py-2 rounded-lg hover:bg-gray-500 transition hover-scale" onclick="toggleEditForm()">Hủy</button>
                                </div>
                            </form>
                        </div>
                    <?php } else { ?>
                        <p class="text-center text-gray-600">Không tìm thấy thông tin người dùng.</p>
                    <?php } ?>
                </div>

                <!-- Cột phải: Lịch sử đơn hàng -->
                <div class="glass-container rounded-lg shadow-lg p-6">
                    <h3 class="text-2xl font-semibold bg-gradient-to-r from-pink-400 to-blue-400 bg-clip-text text-transparent mb-4">Lịch Sử Đơn Hàng</h3>
                    <?php if (mysqli_num_rows($result) > 0) { ?>
                                    ...
                                    <tr class="order-item border-b">
                                        ...
                                        <td class="p-4" data-label="Hành Động">
                                            <?php if (isset($_GET['view_details']) && $_GET['view_details'] == $order['id']) { ?>
                                                <a href="profile.php" class="action-button hover-scale">
                                            ...
                                            <?php } else { ?>
                                                <a href="profile.php?view_details=<?php echo $order['id']; ?>" class="action-button hover-scale">
                                                    <svg class="w-5 h-5 mr-1" fill="none" stroke="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
                                                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 12a3 3 0 11-6 0 3 3 0 016 0z"></path>
                                                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M2.458 12C3.732 7.943 7.523 5 12 5c4.478 0 8.268 2.943 9.542 7-1.274 4.057-5.064 7-9.542 7-4.477 0-8.268-2.943-9.542-7z"></path>
                                                    </svg>
                                ...
                                <?php if ($selected_order['status'] == 'pending') { ?>
                                    <div>
                                        ...
                                        <form method="POST" action="" onsubmit="return confirmCancel(this);">
                                            ...
                                            <button type="submit" name="cancel_order" class="cancel-button hover-scale">
                        ...
                        <?php } else { ?>
                            <div class="text-center">
                                ...
                                <a href="category.php" class="mt-4 inline-block bg-gradient-to-r from-pink-400 to-blue-400 text-white px-6 py-2 rounded-lg hover:from-pink-500 hover:to-blue-500 transition hover-scale">Tiếp Tục Mua Sắm</a>
                            </div>
                        <?php } ?>
                    <?php } else { ?>
                        <p class="text-center text-gray-600">Không có đơn hàng nào để hiển thị.</p>
                    <?php } ?>
                </div>
            </div>
        </div>
